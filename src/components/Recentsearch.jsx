function Recentsearch({ recentHistory, setrecentHistory, setselectedHisotry }) {
  const clearhistory = () => {
    localStorage.clear();
    setrecentHistory([]);
  };
          ...
          {recentHistory &&
            recentHistory.map((item, index) => (
              <div className="flex justify-between pr-3 py-1">
                <li
                  key={index}
                  onClick={() => setselectedHisotry(item)}
                  className=" w-full pl-5 px-5 truncate text-zinc-400 cursor-pointer hover:bg-zinc-700 hover:text-zinc-200"
                >
                  {item}
                </li>
                <button
